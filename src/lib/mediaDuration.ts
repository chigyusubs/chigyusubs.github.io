...
// --- MP4/MOV parsing --------------------------------------------------------
function parseMp4DurationFromBuffer(buffer: ArrayBuffer): number | null {
...
async function probeMp4Duration(file: File): Promise<number | null> {
  ...
  const headDur = parseMp4DurationFromBuffer(head);
  ...
  if (file.size > MP4_READ_BYTES) {
    ...
    const tailDur = parseMp4DurationFromBuffer(tail);
...
function parseWebmDuration(buffer: ArrayBuffer): number | null {
  ...
  const segEnd =
    segSize.value === (1 << (7 * segSize.length)) - 1
      ? view.byteLength
      : offset + segSize.value;
...
function estimateMp3Duration(
  ...
  for (; offset + 4 < view.byteLength; offset += 1) {
    ...
    return durationSeconds > 0 && Number.isFinite(durationSeconds)
      ? durationSeconds
      : null;
  }
...
export async function getMediaDuration(file: File): Promise<number | null> {
  ...
  if (
    ...
    Number.isFinite(mediaElementDuration) &&
    mediaElementDuration > 0
  ) {
