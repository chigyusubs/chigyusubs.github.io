  ...
  return (
    <SectionCard title="Results">
      <div className="flex gap-2 mb-4">
        <Button
          tone="primary"
          onClick={() => downloadText("translated.vtt", result.vtt)}
          disabled={!result.vtt}
        >
          Download VTT
        </Button>
        <Button
          tone="primary"
          onClick={() => downloadText("translated.srt", result.srt)}
          disabled={!result.srt}
        >
          Download SRT
        </Button>
      ...
      {showPreview && result.vtt && (
        <div
          className="mb-4 p-3 rounded border text-base whitespace-pre-wrap max-h-64 overflow-y-auto theme-border bg-theme-code"
        >
          {result.vtt}
        </div>
      )}
            ...
            <details className="mt-2">
              ...
              <div className="mt-2 space-y-2">
                ...
                <div>
                  ...
                  <pre
                    className="p-2 rounded border text-base whitespace-pre-wrap max-h-40 overflow-y-auto theme-border bg-theme-code text-theme"
                  >
                ...
                {chunk.status === "ok" && (
                  <div className="pt-2 flex justify-between items-center">
                    <p className={`text-[10px] ${theme.mutedText}`}>
                      Use this if the chunk has hallucinations or repeated
                      lines.
                    </p>
                    <Button
                      className="text-xs px-2 py-1"
