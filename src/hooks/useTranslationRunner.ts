...
export function useTranslationRunner() {
  ...
  const runTranslation = async (opts: RunOptions) => {
    ...
    const pending: ChunkStatus[] = previewChunks.map((chunk) => ({
      idx: chunk.idx,
      status: "waiting",
      tokens_estimate: Math.max(
        Math.floor(serializeVtt(chunk.cues).length / 4),
        1,
      ),
      warnings: [],
      vtt: "",
        ...
        if (isDebugEnabled()) {
          logDebugEvent({
            ...
            data: {
              ...
              warnings: chunkResult.warnings?.length || 0,
            },
          });
        }
      ...
      if (runToken === runTokenRef.current) {
        setResult((prev) => {
          ...
          const newChunks = prev.chunks.map((c) =>
            c.idx === updated.idx ? { ...c, ...updated } : c,
          );
          const cleanedChunks = newChunks.map((c) =>
            c.idx === updated.idx
              ? { ...c, warnings: updated.warnings || [] }
              : c,
          );
